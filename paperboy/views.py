...
def details(request, id):
    pb = Paperboy.objects.filter(pk=id).first
    earnings = "{:10.2f}".format(Paperboy.total_earnings())
    context = {'pb': pb, 'total_papers': Paperboy.total_papers(), 'total_earnings': earnings}
    return HttpResponse(render(request, 'paperperson_details.html', context))